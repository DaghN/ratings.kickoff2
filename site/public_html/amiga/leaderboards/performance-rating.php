...
<thead>
    <tr>
        ...
        <th<?php echo k2_lb_th(3, $lbSort, ''); ?> data-k2-sort="number" data-k2-help="<?php echo htmlspecialchars($perfHelp, ENT_QUOTES, 'UTF-8'); ?>">Perf. rating</th>
        <th<?php echo k2_lb_th(4, $lbSort, ''); ?> data-k2-sort="number" data-k2-help="Games in the listed event.">Event games</th>
        <th<?php echo k2_lb_th(5, $lbSort, 'k2-table-cell--left'); ?> data-k2-sort="text">Event</th>
        <th<?php echo k2_lb_th(6, $lbSort, 'k2-table-cell--right'); ?> data-k2-sort="number">Date</th>
    </tr>
</thead>
...
foreach ($perfRows as $row) {
    ...
    <tr>
        ...
        <td<?php echo k2_lb_td(3, $lbSort); ?>><?php echo amiga_profile_tournament_rating_cell($row['performance_rating'] ?? null); ?></td>
        <td<?php echo k2_lb_td(4, $lbSort); ?>><?php echo k2_fmt_games_played($eventGames); ?></td>
        <td<?php echo k2_lb_td(5, $lbSort, 'k2-table-cell--left'); ?>><?php
            echo amiga_tournament_link(
                (int) ($row['tournament_id'] ?? 0),
                (string) ($row['tournament_name'] ?? '')
            );
        ?></td>
        <td<?php echo k2_lb_td(6, $lbSort, 'k2-table-cell--right'); ?> data-k2-sort-value="<?php echo amiga_profile_event_date_sort_value($row); ?>"><?php echo amiga_profile_format_event_date($row['event_date'] ?? null); ?></td>
    </tr>
    <?php
    $rank++;
}
