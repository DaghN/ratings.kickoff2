...
$query = 'SELECT p.id AS ID, p.name AS Name, s.Rating, s.NumberGames, s.PeakRating, s.LowestRating, '
    ...
    . amiga_player_base_from_sql() . ' WHERE ' . amiga_lb_player_where_sql() . ' ORDER BY s.PeakRating DESC, s.Rating DESC';
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    <tr>
        ...
        <td><?php echo k2_fmt_peak_rating($row['PeakRating']); ?></td>
        <td><?php echo k2_fmt_nadir_rating($row['LowestRating']); ?></td>
        <td><?php echo k2_fmt_lb_stat($row['AverageOpponentRating'], $games); ?></td>
        <td><?php echo k2_fmt_lb_stat($row['HighestRatedVictim'], $games); ?></td>
        <td><?php echo k2_fmt_lb_stat($row['LowestRatedCulprit'], $games, 5000.0); ?></td>
    </tr>
    <?php
    $rank++;
}
