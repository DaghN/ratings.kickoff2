 ...
 * URL hash targets win: carry restore is skipped and the hash element is scrolled after layout.
...
(function () {
	...
	function scheduleHashScroll() {
		...
		function hashScrollTop(el) {
			...
			return Math.max(0, rect.top + window.scrollY - marginTop);
		...
		function scrollToHash() {
			...
			var top = hashScrollTop(el);

			ensureMinScrollHeight(top);

			var maxTop = Math.max(0, scrollHeightNow() - (window.innerHeight || 0));
		...
		if (typeof ResizeObserver !== 'undefined') {
			...
			var hashResizeObserver = null;

			var hashResizeTimer = null;



			function stopHashResizeWatch() {
				...
				if (hashResizeTimer) {
					...
					clearTimeout(hashResizeTimer);

					hashResizeTimer = null;
			...
			hashResizeObserver = new ResizeObserver(function () {

				scrollToHash();

			});

			hashResizeObserver.observe(document.documentElement);

			hashResizeTimer = setTimeout(stopHashResizeWatch, 2000);
