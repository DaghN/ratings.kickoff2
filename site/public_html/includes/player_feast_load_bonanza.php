 ...
 * Fallback: vs same opponent, highest SumOfGoals among games where hero outscored opponent.
 */
...
/**
 * Highest combined-goal H2H game where the hero outscored this opponent.
 *
 * @return array<string, mixed>|null
 */
function player_feast_load_bonanza_h2h_fallback(mysqli $con, int $playerId, int $opponentId): ?array
{
    if ($playerId < 1 || $opponentId < 1) {
        return null;
    }
    $escPid = (string) (int) $playerId;
    $escOid = (string) (int) $opponentId;
    $gRes = k2_player_feast_query(
        $con,
        'trophy_game_shootout_h2h_fallback',
        "SELECT id, Date, idA, idB, NameA, NameB, GoalsA, GoalsB, ActualScore, AdjustmentA, AdjustmentB, SumOfGoals
         FROM ratedresults
         WHERE (idA = '$escPid' AND idB = '$escOid' AND GoalsB < GoalsA)
            OR (idB = '$escPid' AND idA = '$escOid' AND GoalsA < GoalsB)
...
function player_feast_load_bonanza_trophy(mysqli $con, int $playerId, int $biggestSumGameId): ?array
{
    ...
    $fallbackRow = player_feast_load_bonanza_h2h_fallback($con, $playerId, (int) $parsed['opponent_id']);
    if ($fallbackRow === null) {
        return null;
    }
