...
if ($realm === 'amiga' && $oppCountry !== '') {
    ...
    $heroName = (string) $identity['name'];
    $buckets = amiga_player_h2h_country_goals_scored_buckets($con, $playerId, $countryToken, $ctx, $goalsSide);
    $maxGoals = $buckets === [] ? 0 : (int) $buckets[count($buckets) - 1]['goals'];
    $totalGames = player_goals_scored_distribution_total_games($buckets);
    $avgGoalsPerGame = player_goals_scored_distribution_avg_goals_per_game($buckets);
    mysqli_close($con);
    ...
    echo json_encode([
        ...
        'playerName' => $goalsSide === 'rival' ? $countryLabel : $heroName,
        'opponentName' => $goalsSide === 'rival' ? $heroName : $countryLabel,
        'maxGoals' => $maxGoals,
        'totalGames' => $totalGames,
        'avgGoalsPerGame' => $avgGoalsPerGame,
        'buckets' => $buckets,
