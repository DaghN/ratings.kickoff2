...
if ($realm === 'amiga' && $pairHero !== '' && $pairRival !== '') {
    ...
    $payload = amiga_country_rivals_h2h_scoreline_heatmap_payload($con, $heroToken, $rivalToken, $ctx);
    $totalGames = 0;
    ...
    echo json_encode([
        ...
        'gridAxisMax' => (int) $payload['grid_axis_max'],
...
if ($realm === 'amiga' && $oppCountry !== '') {
    ...
    $payload = amiga_player_h2h_country_scoreline_heatmap_payload($con, $playerId, $countryToken, $ctx);
    $totalGames = 0;
    foreach ($payload['cells'] as $cell) {
        $totalGames += (int) $cell['games'];
    }
...
if ($realm === 'amiga') {
    ...
    $payload = amiga_player_h2h_scoreline_heatmap_payload($con, $playerId, $opponentId, $ctx);
    $totalGames = 0;
    foreach ($payload['cells'] as $cell) {
        $totalGames += (int) $cell['games'];
    }
