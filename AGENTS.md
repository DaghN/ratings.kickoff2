...
- **Amiga derived repair** — batch `*-rebuild` CLIs retired Jun 2026; **canon/writer regression** → **`python -m scripts.amiga prove`**. **Staging community mistakes / cancel / media** → [`amiga-live-ops-platform.md`](docs/amiga-live-ops-platform.md) + [`amiga-live-ops-practice-track.md`](docs/amiga-live-ops-practice-track.md) (drill-first slices; anchored repair, not prove-first). Policy: [`amiga-derived-write-policy.md`](docs/amiga-derived-write-policy.md).
- **Amiga DDL (ko2amiga_db)** — **holy ops only:** edit `sql/ground/`, `sql/structure/`, or `sql/derived/` bundles → **`python -m scripts.amiga prove`**. **Forbidden:** manual `mysql ALTER`, new numbered `sql/047_*.sql`, one-off `apply_schema_*` on live DB ([`amiga-ground-layers-policy.md`](docs/amiga-ground-layers-policy.md) G12; [`amiga-data-contract.md`](docs/amiga-data-contract.md)). **RTB-1:** [`amiga-running-tournament-boundary-implementation-plan.md`](docs/amiga-running-tournament-boundary-implementation-plan.md) § DDL — holy ops only. Precedent: `047_player_source.sql` removed — PC-1 uses `ground/001_core.sql` only.
