...
- **Amiga staging** — **staged `ko2amiga_db` = prod**; **local `ko2amiga_work` = repair shop** (pull → simul/repair → push). Policy: [`amiga-staging-authority-policy.md`](docs/amiga-staging-authority-policy.md). **Pull from staged:** `powershell -ExecutionPolicy Bypass -File scripts\pull_ko2amiga_from_staging.ps1 -Force`. **Side-pull compare:** `-TargetDatabase ko2amiga_staging_cmp` (work untouched). **Push to staged:** run `export_ko2amiga_work.ps1` after **simul** (audits export table list vs `schema_bundles` before dump); remind **pull from staged first** if community events may have changed since last sync. Registry: `scripts/amiga/staging_export_tables.py` · `data/amiga/staging_export_tables.json`. **L5 backup/admin-delete v1 complete** — Case A/B/C delete + seals; Case C forward finalize **must** seed inverse from changelog ([`amiga-player-inverse-count-timeline-policy.md`](docs/amiga-player-inverse-count-timeline-policy.md) §5.3); export/seal data parts = JSON registry only (no second hardcoded dump list). Proof [`amiga-export-inverse-roundtrip-test-plan.md`](docs/amiga-export-inverse-roundtrip-test-plan.md). **Case C insert** (mid-history Finish) — **shipped** on `fixtures.php`; staged proof Phase D pending — [`amiga-case-c-insert-finish-implementation-plan.md`](docs/amiga-case-c-insert-finish-implementation-plan.md). Healthy checkpoint: `work-2026-07-23-inverse-roundtrip`. Runbook: [`amiga-staging-handoff.md`](docs/amiga-staging-handoff.md). Oracle rebuild only: `setup_ko2amiga_db.ps1`. Do **not** ping Steve for routine Amiga SQL re-import.
...
- **DB continuity in git** — sealed database backups **belong in git** (Amiga `data/amiga/day0/`, `data/amiga/checkpoints/`; online the same habit is wanted). Working `data/dumps/` / staging `_import/` SQL may stay gitignored until sealed. **Never** treat “dumps must stay out of git” as policy — that was an agent misunderstanding. **Secrets** (credentials, `*.local.php`, ops ini) still stay out. Authority: [`PROJECT_BRIEF.md`](PROJECT_BRIEF.md) · [`README.md`](README.md) Continuity · [`data/dumps/README.md`](data/dumps/README.md).
