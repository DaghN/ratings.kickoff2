...
- **Amiga derived repair** — batch `*-rebuild` CLIs retired Jun 2026; **canon/writer regression** → **`python -m scripts.amiga prove`**. **Staging community mistakes / cancel / media** → [`amiga-live-ops-platform.md`](docs/amiga-live-ops-platform.md) (anchored repair, not prove-first). Policy: [`amiga-derived-write-policy.md`](docs/amiga-derived-write-policy.md).
