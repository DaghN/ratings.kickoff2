...
- **Amiga work DB kill switches** — [`amiga-modern-ground-platform.md`](docs/amiga-modern-ground-platform.md) §0.1: **simul** ≠ **seed-work** / **prove** / **import-witness**; video `start_sec` edits = **shared** CSV + align (never deploy-only JSON); nuclear ops need `--i-mean-destroy-work` + `--confirm-destroy=destroy-ko2amiga-work`.
