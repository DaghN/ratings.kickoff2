...
- **K2 page chrome nav** — read [`k2-nav-implementation-checklist.md`](docs/k2-nav-implementation-checklist.md), copy nearest include from §1; spacing = `--k2-nav-gap` bottom-only ([`nav-spacing-policy.md`](docs/nav-spacing-policy.md)). **Never** `:has(+ …)` spacing lists or content `margin-top` for nav gaps. **Active pill / page placement:** [`navigation-model.md`](docs/navigation-model.md) NM1–NM6 — hub bar always present; **entity pages** (game/player/tournament/country/milestone) live at the realm root in a **singular** namespace with **no active pill**; player pages **do not** "replace" the hub bar (stale older note).
