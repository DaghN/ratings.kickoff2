...
- **K2 LB URL landing sort (Track A)** — hub wings with HoF/deep links: [`k2-lb-ssr-sort-policy.md`](docs/k2-lb-ssr-sort-policy.md) + [`k2-lb-ssr-sort-implementation-plan.md`](docs/k2-lb-ssr-sort-implementation-plan.md); ~5 wings/slice; **slice 1 shipped** (Jul 2026); **do not rewrite HoF hrefs** — upgrade page + verify column map. Amiga rating LB: Δ always visible + fixed `AMIGA_LB_RATING_COL_*` (SSR-13). Profile mosaic links = Track B ([`player-profile-stat-links-policy.md`](docs/player-profile-stat-links-policy.md)).
