...
- **K2 page chrome nav** — read [`k2-nav-implementation-checklist.md`](docs/k2-nav-implementation-checklist.md), copy nearest include from §1; spacing = `--k2-nav-gap` bottom-only ([`nav-spacing-policy.md`](docs/nav-spacing-policy.md)). **Never** `:has(+ …)` spacing lists or content `margin-top` for nav gaps.
- **Turbo page JS boot** — site uses Hotwired Turbo for gapless jukebox; read [`k2-turbo-page-init-checklist.md`](docs/k2-turbo-page-init-checklist.md). **Never** bare `DOMContentLoaded`-only init on widgets/charts/filters — use `k2OnPageReady` + idempotent root guards. **Hash anchor links** (`#player`, `#k2-lb-table`, `#k2-country-roster`, …): same doc, § Hash anchor landing — use `k2_carry_scroll_restore.php`; do not add page-local scroll JS.
