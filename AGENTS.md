...
- **Amiga staging** — **staged `ko2amiga_db` = prod**; **local `ko2amiga_work` = repair shop** (pull → simul/repair → push). Policy: [`amiga-staging-authority-policy.md`](docs/amiga-staging-authority-policy.md). **When Dagh asks to export to staged:** run `export_ko2amiga_work.ps1` after **simul**; remind **pull from staged first** if community events may have changed since last sync. Runbook: [`amiga-staging-handoff.md`](docs/amiga-staging-handoff.md). Oracle rebuild only: `setup_ko2amiga_db.ps1`. Do **not** ping Steve for routine Amiga SQL re-import.
