...
- **Amiga Opponents wing (Jun 2026):** **W/D/L · Goals · DDs + H2H (slices D+F) shipped** — poster/pickers/pair detail/moments/charts on `amiga/player/opponents/h2h.php`; Amiga `realm=` API branches + event-step rating compare. Policy [`amiga-opponents-wing-policy.md`](docs/amiga-opponents-wing-policy.md). **Country grain (Jun 2026):** policy locked — `opponents/country/*` URLs, horizontal vs Player · vs Country segment, read-time matchup roll-up + country TPR; not shipped yet — [`amiga-opponents-country-grain-policy.md`](docs/amiga-opponents-country-grain-policy.md).
...
| 2026-06-29 | **Amiga WC HoF single-game links** — §4.6 value cells (most goals in one game, biggest win margin, biggest draw, biggest sum of goals) now deep-link to `/amiga/game.php?id=` via stored `*GameID`, not WC goals LB. |
