...
**Scroll mirror (overflow top bar):** `js/k2-table-scroll-mirror.js` activates only when a wrapped table’s `scrollWidth > clientWidth`. Head: `includes/k2_sortable_table_assets_head.inc.php` (mirror on by default; `$k2SortableTableScrollMirror = false` to opt out). Online hub LBs: `includes/k2_lb_sortable_table_head.inc.php`. Markup: `k2_table_wrap_open(true)` / `k2_table_wrap_close()`. Wide tables (hub LBs both realms, games hub Recent/All **both realms**, league period games, player games, **player Opponents W/D/L · Goals · DDs**, **Amiga player tournament history**, **Amiga player Games**, **Amiga tournament catalog index**, **Amiga live tournaments index**, Amiga WC stats/players, tournament event-stats) use mirror; compact/narrow tables (status league, **Games highlights both realms**) do not. **Mobile document width (Jul 2026):** `.k2-table-wrap` is `position: relative` — absolute `.visually-hidden` spans inside `<th>` (e.g. the hidden “Adjustment lost” label) otherwise resolve their containing block outside the scrolling wrap and widen the document, making phones zoom the whole site out. `html`/`body.k2-site` + games-hub wrappers also carry `overflow-x: hidden` as a safety net.
...
| Player Opponents ledger (W/D/L · Goals · DDs) | 1 — Games (online); **2 — Games** (Amiga — Elo col 1) |
