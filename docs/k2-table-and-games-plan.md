...
**Scroll mirror (overflow top bar):** `js/k2-table-scroll-mirror.js` activates only when a wrapped table’s `scrollWidth > clientWidth`. Head: `includes/k2_sortable_table_assets_head.inc.php` (mirror on by default; `$k2SortableTableScrollMirror = false` to opt out). Online hub LBs: `includes/k2_lb_sortable_table_head.inc.php`. Markup: `k2_table_wrap_open(true)` / `k2_table_wrap_close()`. Wide tables (hub LBs both realms, games hub Recent/All **both realms**, league period games, player games, **player Opponents W/D/L · Goals · DDs**, **Amiga player tournament history**, **Amiga player Games**, **Amiga tournament catalog index**, **Amiga live tournaments index**, Amiga WC stats/players, tournament event-stats) use mirror; compact/narrow tables (status league, highlights) do not.
