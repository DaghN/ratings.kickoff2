...
| **FO5** | **Placement** — Table tab, same workspace as Finish (modal, second panel, or gated sub-step immediately before the commit button). One mental place: “table → confirm order → make official.” *(May move with [`amiga-organizer-workspace-simplification-policy.md`](amiga-organizer-workspace-simplification-policy.md) chrome slices — keep confirm next to Finish.)* |
