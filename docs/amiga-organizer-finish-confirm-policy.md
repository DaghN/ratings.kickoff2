...
| **FO5** | **Placement** — Table tab, same workspace as Finish (modal, second panel, or gated sub-step immediately before the commit button). One mental place: “table → confirm order → make official.” *(Kept on Table through [`amiga-organizer-workspace-simplification-policy.md`](amiga-organizer-workspace-simplification-policy.md) chrome — confirm stays next to Finish.)* |
