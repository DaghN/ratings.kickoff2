...
| **Carry-scroll restore** | `k2_carry_scroll_restore.php` + `k2-carry-scroll.js`. **Current (post-Turbo, full-page nav):** a **pre-paint body cloak** (`html.k2-carry-cloak body{visibility:hidden}`) engages **only when** a carry payload with **`y > 0`** or a `#hash` target is pending; scroll is applied inside a rAF loop, then reveal. **`y === 0`:** destination clears payload — **no cloak** (normal load at top); sources still store scroll (realm, TT ribbon, pickers). Hard 700 ms timeout + `load` listener. See [`tt-chrome-baseline-f6-attempt-log.md`](orchestration/tt-chrome-baseline-f6-attempt-log.md) iter 3. ~~(Turbo-era: restore on `turbo:render` + `currentVisit.scrolled=true`, no cloak.)~~ |
