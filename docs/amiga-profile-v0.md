...
- **Rank chart** — **shipped** (slices 1–5 + Jun 2026 tweak session) — profile panel below rating · hint: end-of-day rank after each tournament day · **X:** full Amiga timeline from first tournament ever (`timelineStart` → today; **not** player career span; Y **Career** does not trim X; no in-chart zoom) · peak summary (`Peak: #N` linear / `Peak: P%` percentile + date, first on ties) · `GET /api/player_rank_history.php?realm=amiga&id=` (+ `&as=` TT) · `player-rank-chart.js` · defaults **Linear · Career · stepped** · scale **Linear · Percentile** (log dropped) · contextual window rows · empty band = empty chart (no status) · probe `scripts/oneoff/amiga_rank_history_tt_parity.php` · [`amiga-player-rank-chart-policy.md`](amiga-player-rank-chart-policy.md)
