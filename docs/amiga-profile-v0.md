...
**Stat link policy (hero + mosaic):** [`player-profile-stat-links-policy.md`](player-profile-stat-links-policy.md) — inventory-first; leaderboards for comparison. **Chronology inventories (Opponents + planned victim kinds):** [`amiga-player-chronologies-policy.md`](amiga-player-chronologies-policy.md).
...
| Opponents wing (comparison) | `/amiga/player/opponents/h2h.php?id={id}` (default wing) · `wdl.php` · `goals.php` · `dds.php` — H2H / ledger; **not** mosaic Opponents count |
| Opponents chronology (inventory) | `/amiga/player/chronologies/opponents/made-it.php?id={id}` · `graphs.php` — [`amiga-player-chronologies-policy.md`](amiga-player-chronologies-policy.md) |
...
- **LB wing slices (Jul 2026)** — eight profile panels in a balanced **3-column grid** — **Activity** (last/first tournament + last/first World Cup → tournament entity links + stacked event dates; **shipped** v0) then seven LB-wing mini-panels (Results+Goals · DDs+Victims · Honours+Calendar+Peak); each panel titled **`h3.k2-panel-heading` above the table** (Moments pattern — not in-table `server-records-section-header`); **Goals mosaic labels (Jul 2026-20):** written-out row text (not LB abbreviations) — Goals scored / conceded / Goal difference / per-game averages / Goal ratio / Most goals scored·conceded / Biggest winning·losing margin / Highest total goals / Biggest draw; **DDs & CSs panel:** title **Double Digits & Clean Sheets**; ratio rows written out; conceded block keeps DD/CS abbr; **Victims & Culprits:** standalone Victims·Culprits capitalized; mid-line lowercase (Double Digits / Clean Sheets / MGC victims·culprits; Biggest loss victims · Biggest win culprits written out); MGC/MGS stay abbr + expanded tooltip; **Tournament honours:** Tournaments played · Perfect tournaments; **Calendar & geography:** Most games / tournaments in one year · Different host countries; peak year+count inline (muted year, wide gap); **Peak rating:** Peak rating / Peak rank / Rating nadir dates inline the same way; **Results → Games** links to player Games tab `#matching-games` (same as hero; Jul 2026-15); **Calendar & geography** peak games/events → Calendar-geo LB; **Peak rating** Peak Rating → establishing tournament + tooltip; Peak rank → rating LB TT snapshot + tooltip; nadir → peak-rating LB col 8; Highest Victim / Lowest Culprit → Games tab `result=win|loss` + `sort=opp_rating` (Jul 2026-15); **Calendar & geography** peak games/events show calendar year inline before the count (muted year, wide gap); geo country counts (host / faced / beaten / beaten by) **not yet linked** — Track B backlog; **Opponents** count → opponents **chronology** Made it ([`amiga-player-chronologies-policy.md`](amiga-player-chronologies-policy.md)); distinct from **Opponents wing** (H2H comparison). HoF-style `k2-table server-records-table k2-table--calm-stats` per panel with `data-k2-anchor-col="1"`. **Layout (Jul 2026-15):** content below player nav uses **full `--k2-max-width` (1200px)** — site `body.k2-site` `20px` horizontal gutter only (no per-section `1.25rem`); mosaic column gap **`--k2-amiga-profile-lb-slice-col-gap` 18px**, stack gap **`--k2-amiga-profile-lb-slice-gap` 22px** (`player-feast-sections.css`). **Time-travel compliant** under `as=` — snapshot row at cutoff + boundary reads ≤ cutoff (`includes/amiga_profile_lb_slices.php`). Stat link register: [`player-profile-stat-links-policy.md`](player-profile-stat-links-policy.md).
...
**Deferred on profile:** dedicated WC medals block, activity calendars; **feast parity** (heatmaps, story bands, rivalry teaser — gestating **C01** in [`creative-ideas-july-2026.md`](creative-ideas-july-2026.md); surrounding Activity/News/Opponents infra is already live — gap is framing, not writers). **Opponents wing** shipped (`amiga/player/opponents/*` — W/D/L · Goals · DDs + **full H2H** poster/pair detail/moments/charts) — sortable ledger via `amiga_player_opponents_tables.php` (cloak on parent page; Tier B). Wide-table stack: [`k2-table-implementation-checklist.md`](k2-table-implementation-checklist.md). See also [`amiga-surface-expansion-overview.md`](amiga-surface-expansion-overview.md) §4.
