...
- **LB wing slices (Jul 2026)** — three merged label | value tables in a wrapping mosaic (Rating+Goals · DDs+Victims · Honours+Calendar+Peak); `border-radius: var(--k2-radius-md)`; present-only (`includes/amiga_profile_lb_slices.php`). Perf. rating omitted. Legacy Career / Honours / Perf highlight below.
