...
- **LB wing slices (Jul 2026)** — three merged label | value tables in a wrapping mosaic (Results+Goals · DDs+Victims · Honours+Calendar+Peak); HoF-style `k2-table server-records-table k2-table--calm-stats` with `data-k2-anchor-col="1"` and in-table `server-records-section-header` dividers per LB wing (first wing titled **Results**, not Rating); **time-travel compliant** under `as=` — snapshot row at cutoff via [`amiga-profile-time-travel-policy.md`](amiga-profile-time-travel-policy.md) (`includes/amiga_profile_lb_slices.php`).
