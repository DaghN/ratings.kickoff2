...
| **Generate dump** | Preview page → **Generate dump** (admin POST / session) |
| **Download dump** | Preview page → **Download dump** |
...
| Step | URL |
|------|-----|
| **Preview** (no DB changes) | https://ratings.kickoff2.com/amiga/run_import_ko2amiga.php?once=ko2amiga-import-one-shot |
| **Apply import** | Preview page → **Apply import** (admin POST / session; multi-part) |
...
Passwords are in `amiga/_ops/amiga_ops_password.local.php` (gitignored; WinSCP-deployable — staging `/config` is root-owned): **`$admin_password`** (import/export) and **`$organizer_password`** (fixtures). Type them on the POST prompt page — do not bookmark `?pwd=`.
