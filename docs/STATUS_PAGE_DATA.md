...
| **v1.5 live room** | **Shipped (Jul 2026)** — 1 s pulse, cascade on rated finish, client half clocks, **minimal glow** (Online new player, live/recent new game names + recent score digits, live goal white bloom), live games **`StartTime ASC`** (newest bottom), visibility catch-up, SRL-16 rating re-sort — [`status-room-live-policy.md`](status-room-live-policy.md) § Glow contract · § Production readiness |
