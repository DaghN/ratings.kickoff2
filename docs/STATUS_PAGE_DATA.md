...
| **v1.5 live room** | **Shipped (Jul 2026)** — 1 s pulse, cascade on rated finish, **`live_clocks` every heartbeat** (SRL-9 smart anchor — keeps 1 s tick when prod `HalfCountdown` is sparse), client half clocks, **cascade glow** (active LB Elo for rating gainers; league Activity **Games** + Points **Pts** on finishers), minimal lobby glow, live **`StartTime ASC`** (newest bottom), visibility catch-up, SRL-16 rating re-sort — [`status-room-live-policy.md`](status-room-live-policy.md) § Glow contract · § Production readiness |
