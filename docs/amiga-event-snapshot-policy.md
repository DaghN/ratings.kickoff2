...
| **S12** | **Inverse victim/culprit counts (exception to S5)** | `MostGoalsScoredCulprits`, `BiggestWinCulprits`, `MostGoalsConcededVictims`, `BiggestLossVictims` on a player's sparse snapshot row are **not** TT-authoritative when the player did not participate in the event where the count changed. **Proposed:** separate sparse changelog timeline — [`amiga-player-inverse-count-timeline-policy.md`](amiga-player-inverse-count-timeline-policy.md). Chronology pointer inventory reads remain correct via other players' snapshots. |
