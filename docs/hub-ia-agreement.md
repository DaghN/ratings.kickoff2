...
| Tint picker | Closed by default behind **Tint** disclosure (hub + player nav); closes when navigating anywhere on the site. |
...
**Section chapter (Jun 2026):** `includes/k2_hub_chapter.inc.php` — title + lede above sub-nav. Online: **Online activity** chapter title on `activity.php` (hub tab label still **Activity**), Leaderboards (`lb_nav.php`), Milestones (hub chapter lede; catalog count on Catalog view intro), HoF. Amiga: **World Cups** (Christmas lede + wing map list; snapshot-aware Covid copy), Tournaments, Live tournaments, HoF — **not** Amiga Leaderboards (sub-nav only). **Amiga hub (T13–T19):** Present — **News · Leaderboards · World Cups · Tournaments · Countries · Games · Activity · Hall of Fame · Live** (last). **News** = present landing. **World Cups** universe — [`amiga-world-cups-hub-policy.md`](amiga-world-cups-hub-policy.md) (four wings shipped). **Player stats** = hub wing 3 only (LB duplicate **retired** Jun 2026). **Time travel** hub bar = **Leaderboards · World Cups · Tournaments · Countries · Games · Activity · Hall of Fame** (present hub minus editorial T13: News · Live · future Misc). **Amiga Games hub** — [`url-routes.md`](url-routes.md) `amiga/games/{recent,highlights,all}.php`; mirrors online Games hub (Recent · Highlights · All games). Player wings target full cutoff lens (T15). Header **Present day | Time travel** — **T19** fixed homes (Present → News; Time travel → rating LB + `as=`); **temporal stamp** then **snapshot ribbon** at top of page nav, then hub chapter — [`amiga-time-travel-policy.md`](amiga-time-travel-policy.md) §5.0–5.1.
