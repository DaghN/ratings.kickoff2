...
| Header player search | Cross-realm: `api/player_search.php?realm=all` — Online + Amiga 500 in one dropdown; each hit labelled **Online** or **Amiga** and links to the correct profile (`/player/profile.php` vs `/amiga/player/profile.php`). **Amiga picks carry active `as=`** when time travel is on (`player-search.js`, T16). No realm picker on the search field. In-page pickers (e.g. H2H opponent) stay `realm=online`. |
...
**Section chapter (Jun 2026):** `includes/k2_hub_chapter.inc.php` — title + lede above sub-nav. Online: **Online activity** chapter title on `activity.php` (hub tab label still **Activity**), Leaderboards (`lb_nav.php`), Milestones (hub chapter lede; catalog count on Catalog view intro), HoF. Amiga: Leaderboards (`amiga_lb_nav.php`), Tournaments, Live tournaments, HoF. **Amiga hub (T13–T19):** Present — **News · Leaderboards · World Cups · Activity · Hall of Fame · Tournaments · Live tournaments** (last). **News** = present landing. **World Cups** universe — [`amiga-world-cups-hub-policy.md`](amiga-world-cups-hub-policy.md) (four wings shipped). **Player stats** = same tables on hub wing 3 and LB World Cups wing (shared body). **Time travel** hub bar = **Leaderboards · World Cups · Activity · Hall of Fame** (T13b + WCH12) — contiguous block matches present order after News. Player wings target full cutoff lens (T15). Header **Present day | Time travel** — **T19** fixed homes (Present → News; Time travel → rating LB + `as=`); **temporal stamp** then **snapshot ribbon** at top of page nav (hub chapters suppressed on snapshot tabs) — [`amiga-time-travel-policy.md`](amiga-time-travel-policy.md) §5.0–5.1.
