...
- **Amiga time travel chrome (when `as=` active):** header **Present day | Time travel** → **temporal stamp** (LED date + kicker) → **snapshot ribbon** (Year · Month · Event) → hub/player content. Present mode: no stamp/ribbon; hub chapters stay. Intent + locked rules: [`amiga-time-travel-policy.md`](amiga-time-travel-policy.md) §5.0.
