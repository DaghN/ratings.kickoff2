...
**Checkpoint 2026-06-07:** Foundation and internal-ops guardrails (jobs 001-011) are complete, the Dagh-assisted staging refresh is recorded in job 012, the read-only public live view landed in job 013, and browser ops parity for entrants/stages/fixture assignment landed in jobs 014-017. Worker 018 defined the organizer-first browser workflow; next work should implement the first shell/navigation slice.
...
8. **Browser organizer shell** — tabs, create redirect, and player picker for league creation. See [`prompt-019-browser-organizer-shell.md`](prompt-019-browser-organizer-shell.md).
9. **Staging/code refresh and ops smoke** — Dagh syncs `site/public_html/` to staging, then spot-checks live public and ops flows.
10. **Format capability model** — Swiss, group+knockout promotion, World Cup class, honours. Design checkpoint after demo path stable.
