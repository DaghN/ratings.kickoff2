# Ladder engine & replay — plan and intent (archived)

> **Do not run commands in this doc for cutover or staging proof.** Forward path: [`../coordination/cutover-readiness.md`](../coordination/cutover-readiness.md) · **`php ops/run_ops_sim.php`** on **`kooldb1`** / **`ko2unity_work`**.  
> **Jun 2026:** Post-game ops / simul → [`../ladder-ops-platform.md`](../ladder-ops-platform.md) + [`../work-db-prepare.md`](../work-db-prepare.md). This file is **Python replay / Elo sandbox** history only.

**Status:** **P0–P2 done** (May 2026) — local **`ko2unity_db`** + legacy staging **`kooldb`** one-shot replay (frozen). **Deferred:** P3–P5 (Amiga/offline, prod C++ alignment). **Commands:** `scripts/ladder/README.md`; May staging record: [`STAGING_REPLAY-2026-05.md`](STAGING_REPLAY-2026-05.md); logistics: `PROJECT_MEMORY.md`.
...
- When live per-game behaviour should change on **prod**: document in **`docs/website-data-contract.md`**; Steve merges C++ at cutover from contract + `docs/ratings_cpp.txt`. Records: **`docs/coordination/records-post-game-exception.md`**. No per-table `cpp-snippets/` in repo (retired May 2026).
