...
| REP-008 | Player milestones | Rebuild `player_milestones`: game keys from `ratedresults` + league wave from `player_league_award` (run after REP-012) | Done May 2026 | **Done** (May 2026) | **Pending** | `scripts/ladder/sql/player_milestones_rebuild.sql` (110 keys); **`giant_slayer` rule fix May 2026:** run `player_milestones_rebuild_giant_slayer.sql` on staging/prod after regen |
| REP-014 | Milestone definitions catalog | Load `milestone_definitions` from seed JSON | **Done** (May 2026) | **Pending** | **Pending** | `scripts/oneoff/load_milestone_definitions.py` after SCH-011 |
