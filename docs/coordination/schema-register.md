...
| SCH-004 | `003_player_period_games.sql` | Create `player_period_games` day/month/year activity aggregate table | **Done** | **Done** | **Pending** | Yes (backfill/rebuild) | Staging applied May 2026; handoff `player-period-games-handoff.md` |
| SCH-005 | `004_status_performance_and_monthly_league.sql` | Add Status performance indexes (`ratedresults.Date`, live `resulttable`) + `player_monthly_league` aggregate table | **Done** | **Done** | **Pending** | Yes (monthly league rebuild) | Staging applied May 2026; REP-004 done on staging |
...
| SCH-008 | `007_stored_truth_expansion.sql` | Create `player_period_league`, `player_milestones`, `player_matchup_summary`, `server_period_game_totals`, `server_period_matchups` | **Done** | **Pending** | **Pending** | Yes (rebuild scripts per table) | Contract `docs/website-data-contract.md`; local REP done |
