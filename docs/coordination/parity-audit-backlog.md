...
| **Next step** | Fill [Post-audit triage](#post-audit-triage-empty-until-review) — prioritize **AUD-004** ops orchestration, then deferred writer/display items. |
...
| **Observation — other “not per game” (non-exhaustive)** | From contract / registers — likely **omitted from default simul** unless Mode C or batch: **`perfect_day` / `nightmare_day`** (UTC day-close); **`entered_arena`** (register only); **league period milestone SQL**; **rating fade** (PER-001); future **league finalize** on period boundaries beyond awards; any CMD not yet in [`ops-dispatch.md`](ops-dispatch.md) / [`periodic-register.md`](periodic-register.md). **Needs full inventory** during ops audit. |
...
| **Planned work (owner, post-audit item)** | **Comprehensive audit + fix** of daily-ops vs simul: inventory all derived writers → map to **dispatch CMD** / schedule → implement **single orchestrated simul** (likely Mode C + any missing incremental milestone/league hooks); align Steve runbook. **Out of scope for parity audit pass** until that lands. |
| **Options (first look)** | **(a)** **Orchestrator** CLI: `replay-to` + stepped `FinalizeLeagueDue` (or `rebuild-all` once) + day-close jobs + register seed — **no** `player_milestones_rebuild.sql` on happy path. **(b)** Extend **`timeline_sim`** to full history + document as the one-click simul. **(c)** Incremental **league milestone** writes in finalize (close P6 gap). **(d)** Keep batch rebuilds for **dev parity only**, labelled non-prod-shaped. **(e)** Update **periodic-register** + dispatch registry with complete checklist. |
| **Open questions** | Full CMD list vs contract; whether PER-001 fade belongs in simul; Python ladder tail still batch-finalizing fields; `rebuild-aggregates` without `k2_league_sync_win_milestones`; verification gates (“simul complete” checklist). |
| **Links** | [`work-db-prepare.md`](../work-db-prepare.md) §5.1–5.2; [`staging-work-steve-handoff.md`](staging-work-steve-handoff.md) §4–5; [`ops-dispatch.md`](ops-dispatch.md); [`periodic-register.md`](periodic-register.md); [`post-game-php-development.md`](../post-game-php-development.md) §2.2–2.3; `run_process_game.php`, `run_finalize_league.php`, `run_timeline_sim.php`; `includes/league_standings.php` (`k2_league_sync_win_milestones`); `scripts/ladder/sql/player_milestones_rebuild.sql`. |
| **Resolution** | *(post-audit — blocked on ops pipeline audit/fix)* |
