...
| Amiga career `elo_rank` at finalize | L1 | `032` + **`041`** peak rank | — | **Done** local Jun 2026 (`prove` green) | **Not executed** | — | Hero UI reads `elo_rank`; profile rank chart; **peak rank** on peak-rating LB (`peak_elo_rank` + tournament id, TT-safe); **Peak date** / **Peak rank date** columns (event-level — not `PeakRatingGameID`) |
