...
**Status:** Hygiene slice A **Jun 2026** — league + catalog paths migrated to `ops/`; superseded league rebuild **removed**; one-offs **archived** in file headers.  
...
| `load_milestone_definitions.php` | REP-014 — seed `milestone_definitions` | `milestones-README.md`, cutover packet | `ops/run_prepare.php seed-catalog` | **Superseded** (deprecated header; staging-data path until ops on server) | SCH-010 / prepare |
| `patch_milestone_catalog_copy.php` | Catalog display copy patches (JSON) | `milestones-README.md`, `milestones-add-one-playbook.md` | `scripts/oneoff/` or rare `ops` one-shot | **One-off archive** | OO-* |
| ~~`run_league_awards_rebuild.php`~~ | REP-012 + REP-013 | cutover packet (historical) | `ops/run_finalize_league.php rebuild-all` | **Removed Jun 2026** | REP-012, REP-013 |
...
| `run_player_milestones_diversity_merchant_fix.php` | Surgical fix REP-008b | `milestones-staging-diversity-merchant-fix.md` | Same as full rebuild path | **One-off archive** | feature-log |
...
|------|---------|-------------------------|
| `scripts/finalize_league_periods.php` | **Superseded** (delegate) | `ops/run_finalize_league.php` |
...
1. **Docs only** — **Done (Jun 2026).** PER-003 / REP-012 → `ops/run_finalize_league.php`.
2. **Retire** `scripts/finalize_league_periods.php` — **Done:** thin delegate to ops `--target local-dev`.
3. **Superseded** `load_milestone_definitions.php` — **Done (Jun 2026):** deprecated; ops `seed-catalog`; file kept for staging-data WinSCP path.
4. **League staging rebuild** — **Done (Jun 2026):** deleted `run_league_awards_rebuild.php`.
5. **One-off archives** — **Done (Jun 2026):** `@archived` headers on surgical milestone scripts + `patch_milestone_catalog_copy.php`; delete files when cutover packets closed.
6. **`run_player_milestones_rebuild.php`** — Last large piece; retire when PHP post-game milestones on staging/prod (feature-log).
7. **`dispatch.php`** — Optional consolidation of `run_*.php` (separate slice).
...
| Register | When |
|----------|------|
| [`replay-register.md`](replay-register.md) | REP-012/014/015 path changes |
...
*Update rows when a slice completes (add **Done** date + PR).*
