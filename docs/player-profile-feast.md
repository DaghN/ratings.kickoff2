...
**Clickable hero + mosaic stats:** [`player-profile-stat-links-policy.md`](player-profile-stat-links-policy.md) — inventory-first on profile; leaderboards for comparison.
...
1. Site header + **realm hub bar** (`player_wing_hub_nav.inc.php` / `amiga_player_wing_hub_nav.inc.php`; no hub tab active; tint picker on hub bar only) + **hero** (zero-height `#player` anchor immediately above hero — **inbound** player name links append `#player` so hero lands at viewport top; **wing pills** Profile · Games · … omit hash and use carry-scroll; avatar + name → Profile; rank + rating → Rating LB `#k2-lb-player-{id}`; games → Games tab (`player-games` / `amiga-player-games`) `#matching-games`; **milestones** → four tier-colored counts (all tiers incl. zero, space-separated, garden tier anchors; pure tier ink in `player-hero-rank.css`); Rank/Rating/Games use link-star + accent glow via `player-hero-rank.css`; **panel + avatar** use accent border + H2H-style glow in `theme.css` — country/tournament heroes stay quiet surface) + **player nav** (`--k2-nav-gap` below player nav; feast hero uses **`--k2-player-hero-chrome-gap` (24px)** on hub bar bottom + hero bottom and **`--k2-player-hero-glow-inset` (14px)** left inset in `body.k2-player-wing` — `theme.css`)
4. **At a glance** — three columns × four rows: **Presence** (first/last rated game, days & games this year), **Career** (opponents, games, wins, goals — no DDs, no ranks), **Achievements** (milestones + gold/silver/bronze). Value cells (right column) use **link-star** ink; **Achievements → Milestones** row keeps tier-colored counts (pitch/chrome/amber/holo). Narrow viewports: columns stay side-by-side; panel scrolls horizontally if needed (no vertical stack).
...
10. **Moments** — no visible heading; warm lede; giant-killing + trophy mosaic (accent tag, primary-weight label, link-star scoreline, muted meta); linked games/opponents. **Total goals bonanza:** show `BiggestSumOfGoalsGameID` only when opponent scored **&lt; 3×** hero; else highest-`SumOfGoals` career game where the ratio passes (global walk, not H2H); omit if none (`player_feast_load_bonanza.php`). **Peak rating (last card):** `PeakRating` + `PeakRatingGameID` from `playertable` (`player_feast_load_peak_moment()`); value = rounded peak Elo linked to establishing game; meta = outcome · opponent · date (same as game trophies); omitted when peak unset or game row missing.
11. **Charts** — warm lede before career charts (rating by date / by game # toggle, games per month, goals-per-game histogram); sr-only **Career rating** title; per-panel `k2-chart-block__hint` unchanged inside frames. **Rating by date (online):** hover shows played-days game list (≤8 games, UTC day via `player_calendar_day_games.php`) + summary `{N} rated games. Rating at day end: {rating}` — **link-star** emphasis, shared with played-days heatmap tooltips (`player-calendar-day-tooltip.js`). **Rating by game # (online):** same tooltip shell + game list row; title = calendar date; body = `Game:` + link-star game number, then one scoreline row. **Goals per game** hint: “How many games {name} scored exactly 0, 1, 2… goals in.” (link-star name) + after load “{name} has averaged X goals per game so far.” (`avgGoalsPerGame` from distribution API). Career chart stack + opponents chart centred in page column (960px max). **Games per month** panel anchor `#games-per-month`. **Games per month:** click a bar → **Games** tab `?from=profile-games-chart&period=month&anchor=YYYY-MM-01#day-games` (**← Games per month** back link). **Goals histogram:** click bar → games tab GF filter (`#k2-player-games-filters`). **Coarse pointer** on drill-down bars: first tap = pinned bar + external tooltip; second tap = navigate (shared `k2-coarse-tap.js`; Chart.js built-in tooltips stay off on coarse per `chart-theme.js`).
...
| Moments | `playertable` extreme game IDs + single-row `ratedresults` lookups; career peak = `PeakRating` + `PeakRatingGameID` (`player_feast_load_peak_moment()`) |
...
| Milestones on profile | **Partial** — hero tier counts (garden tier links); full garden on Milestones tab |
