...
10. **Moments** — no visible heading; warm lede; giant-killing + trophy mosaic (accent tag, secondary label/scoreline, muted meta); linked games/opponents. **Total goals bonanza:** show `BiggestSumOfGoalsGameID` only when opponent scored **&lt; 3×** hero; else highest-`SumOfGoals` H2H game vs that opponent where hero outscored them; omit if none (`player_feast_load_bonanza.php`).
11. **Charts** — warm lede before career charts (rating by date / by game # toggle, games per month, goals-per-game histogram); sr-only **Career rating** title; per-panel `k2-chart-block__hint` unchanged inside frames. **Games per month** panel anchor `#games-per-month`. **Games per month:** click a bar → **Games** tab `?from=profile-games-chart&period=month&anchor=YYYY-MM-01#day-games` (**← Games per month** back link). **Goals histogram:** click bar → games tab GF filter (`#k2-player-games-filters`). **Coarse pointer** on drill-down bars: first tap = pinned bar + external tooltip; second tap = navigate (shared `k2-coarse-tap.js`; Chart.js built-in tooltips stay off on coarse per `chart-theme.js`).
12. **Most played opponents** — horizontal bar chart; click a bar opens **Opponents → Head-to-head** for that pairing (coarse: two-tap preview then navigate). Cumulative H2H + rating comparison charts live on the H2H tab — see [`player-opponents-h2h-poster.md`](player-opponents-h2h-poster.md). H2H goals histograms + total-goals chart use the same coarse two-tap drill-down.
