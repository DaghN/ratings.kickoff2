...
11. **Charts** — warm lede before career charts (rating by date / by game # toggle, games per month, goals-per-game histogram); sr-only **Career rating** title; per-panel `k2-chart-block__hint` unchanged inside frames. **Rating by date (online):** hover shows played-days game list (≤8 games, UTC day via `player_calendar_day_games.php`) + summary `{N} rated games. Rating at day end: {rating}` — fixed chart amber (`k2-link-star--chart-amber`), shared with played-days heatmap tooltips (`player-calendar-day-tooltip.js`). **Rating by game # (online):** same tooltip shell + game list row; title = calendar date; body = `Game:` + amber number, then one scoreline row. **Goals per game** hint: “How many games {name} scored exactly 0, 1, 2… goals in.” (link-star name) + after load “{name} has averaged X goals per game so far.” (`avgGoalsPerGame` from distribution API). Career chart stack + opponents chart centred in page column (960px max). **Games per month** panel anchor `#games-per-month`. **Games per month:** click a bar → **Games** tab `?from=profile-games-chart&period=month&anchor=YYYY-MM-01#day-games` (**← Games per month** back link). **Goals histogram:** click bar → games tab GF filter (`#k2-player-games-filters`). **Coarse pointer** on drill-down bars: first tap = pinned bar + external tooltip; second tap = navigate (shared `k2-coarse-tap.js`; Chart.js built-in tooltips stay off on coarse per `chart-theme.js`).
