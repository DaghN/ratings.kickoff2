...
1. Site header + **realm hub bar** (`player_wing_hub_nav.inc.php` / `amiga_player_wing_hub_nav.inc.php`; no hub tab active; tint picker on hub bar only) + **hero** (avatar + name → Profile tab; rank + rating → Rating LB `#k2-lb-table` (zero-height anchor flush above table); games → Activity peaks (games sort); milestones count → Milestones LB — same table-top anchor) + **player nav** (6px hero→nav, 12px nav→content on all `k2-player-wing` tabs — `theme.css`)
