...
| Player name hover glance (online + Amiga) | `js/amiga-player-glance.js`, `includes/amiga_player_glance_config.php`, `api/player_glance.php`, `api/amiga_player_glance.php` | Read-only hero popover; **opt-in** `data-k2-player-glance` / `data-k2-amiga-player-glance` on `k2_player_link()` / `k2_amiga_player_link()`; **`data-k2-amiga-player-glance-rating`** / **`data-k2-player-glance-rating`** on Amiga/online `*_lb_rating_cell_link()` (name + Amiga flag, rank + rating, footer “Click to view rating leaderboard”); opaque canvas instant, print + shadow fade **850ms**; fetch on hover; **500ms** show gate; tier **B** default; `?k2_glance=A\|B` |
