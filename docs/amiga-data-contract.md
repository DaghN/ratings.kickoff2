...
| `amiga_player_tournament_participation` | Derived | Batch: `replay` / `participation-rebuild`. Live: Python `finalize_tournament` (`refresh_tournament_participation_stack`) and PHP `amiga_ops_participation_refresh_tournament` after standings on tournament finalize. **Columns:** event-wide W-D-L + `event_points` from `amiga_games`; `overall_position` from standings overall only; **phase points not stored** (see universe contract §5.2.1) | **Active** |
