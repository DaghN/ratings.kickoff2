...
- **Import closure (target):** `python -m scripts.amiga run` dispatches `disposition_register.json` handlers. **Today:** tier D spec on import; `pure_knockout` + `pure_rr` via CLI until slice 10. See [`amiga-tournament-structure-handlers.md`](amiga-tournament-structure-handlers.md).
