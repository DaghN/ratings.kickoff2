...
- Standings scope resolution prefers fixture metadata when `amiga_games.fixture_id` is present: `league` stages feed `league` standings scopes, `group` fixture stages feed labeled `league` scopes, and `knockout` / `placement` stages feed per-pair `knockout` scopes. If `fixture_id` is NULL, `scripts/amiga/tournament_phases.py` remains the legacy parser.
