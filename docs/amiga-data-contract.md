...
- **Live path (current code — superseded by policy):** software creates scheduled fixtures first; `fixtures record-result` (or ops) **today** inserts `amiga_games` immediately. **Target (locked):** scores on fixture running columns until **Make official** — [`amiga-running-tournament-boundary-policy.md`](amiga-running-tournament-boundary-policy.md).
...
- `python -m scripts.amiga fixtures record-result` — **today** inserts `amiga_games` + standings rebuild. **Target:** fixture running columns only until Make official ([`amiga-running-tournament-boundary-policy.md`](amiga-running-tournament-boundary-policy.md) RTB1–RTB3). Both fixture players must be active (`registered`) tournament entrants before insert.
...
- **Live result entry:** browser `/amiga/ops/fixtures.php` or `fixtures record-result` — **today** writes `amiga_games` + tournament standings. **Target:** running package only until Make official ([`amiga-running-tournament-boundary-policy.md`](amiga-running-tournament-boundary-policy.md)).
