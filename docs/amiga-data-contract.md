...
- `python -m scripts.amiga fixtures list|detail` are read-only schedule inspection commands. `fixtures set-players` assigns participants to scheduled placeholder fixtures only when both players are active (`registered`) tournament entrants, already belong to the tournament stage players, and no game is attached. `fixtures create-fixture` enforces the same entrant rule when `player_a_id` / `player_b_id` are non-null. `fixtures attach-game` is the guarded path for linking pre-existing unattached games to scheduled fixtures (see attachment rules above); prefer `record-result` for new fixture-backed results.
- `/amiga/ops/fixtures.php` is the matching password-gated browser for internal ops; it may create kitchen-marathon tournaments, list/search/manage entrants on generated tournaments, place registered entrants into stages, assign scheduled placeholder fixture players, and record scheduled fixture-backed results, but remains internal tooling rather than public UI. Assignment and result entry refuse withdrawn, replaced, or non-entrant players with a clear error.
...
- **Ops (browser fixture assignment):** on generated tournaments, incomplete scheduled fixtures without attached games show stage-scoped player selects populated from that fixture's stage players (preselecting any slot already assigned). POST `assign_players` still calls `amiga_fixture_assign_players` with the same guardrails as `fixtures set-players` (active `registered` entrants, tournament stage-player membership, distinct players, scheduled status only, refuses fixtures with attached games). Numeric player-id inputs remain as fallback when a stage has fewer than two stage players. Assignment does not require `running` lifecycle.
