...
| `amiga_player_tournament_participation` | Derived | Batch: `replay` / `participation-rebuild`. Live: Python `finalize_tournament` (`refresh_tournament_participation_stack`) and PHP `amiga_ops_participation_refresh_tournament` after standings on tournament finalize. **Roster:** every `(player_id, tournament_id)` with ≥1 `amiga_games` row. **Columns:** event-wide W-D-L + `event_points` from games rollup; **`event_finish_position`** (nullable holistic finish — [`amiga-tournament-honours-rules.md`](amiga-tournament-honours-rules.md) v2, WC podium 1/2/3); **`is_winner`** = finish 1; **`best_knockout_phase`** (KO depth label); **no `wc_medal`** (dropped `022`); phase points/ranks not on participation (standings only) | **Active** |
| `amiga_tournament_finish_override` | Curated | Manual import / ops; read during participation rebuild (Tier E). PK `(tournament_id, player_id)` → `event_finish_position`. Migration `019`. Empty by default | **Active** |
| `amiga_player_tournament_totals` | Derived | Re-aggregated from participation (`rebuild_totals_for_players` / `amiga_ops_participation_rebuild_totals_for_players`). **v2 (complete Jun 2026):** `event_gold`/`event_silver`/`event_bronze`/`event_podiums` + `wc_played`/`wc_gold`/`wc_silver`/`wc_bronze`/`wc_podiums` from `event_finish_position`; `tournaments_won` = `event_gold`; dropped `cup_*`. Read: `/amiga/leaderboards/tournament-honours.php` | **Active** |
...
DDL: [`scripts/amiga/sql/001_core.sql`](../scripts/amiga/sql/001_core.sql), Track B [`002_tournament_standings.sql`](../scripts/amiga/sql/002_tournament_standings.sql), index aggregates [`004_tournament_catalog_stats.sql`](../scripts/amiga/sql/004_tournament_catalog_stats.sql), format foundation [`005_tournament_formats.sql`](../scripts/amiga/sql/005_tournament_formats.sql), fixture foundation [`006_tournament_fixtures.sql`](../scripts/amiga/sql/006_tournament_fixtures.sql), entrant foundation [`007_tournament_entrants.sql`](../scripts/amiga/sql/007_tournament_entrants.sql), lifecycle foundation [`008_tournament_lifecycle.sql`](../scripts/amiga/sql/008_tournament_lifecycle.sql), rating events [`009_rating_events.sql`](../scripts/amiga/sql/009_rating_events.sql), player universe [`010_player_tournament_participation.sql`](../scripts/amiga/sql/010_player_tournament_participation.sql) · [`011_player_tournament_totals.sql`](../scripts/amiga/sql/011_player_tournament_totals.sql) · [`012_player_matchup_summary.sql`](../scripts/amiga/sql/012_player_matchup_summary.sql) · [`013_generalstats.sql`](../scripts/amiga/sql/013_generalstats.sql) · participation `event_points` rename [`014_participation_event_points.sql`](../scripts/amiga/sql/014_participation_event_points.sql) · performance rating [`015_performance_rating.sql`](../scripts/amiga/sql/015_performance_rating.sql) ([`amiga-performance-rating.md`](amiga-performance-rating.md)) · avg goals [`016_participation_avg_goals.sql`](../scripts/amiga/sql/016_participation_avg_goals.sql) · event finish columns [`017_event_finish_position.sql`](../scripts/amiga/sql/017_event_finish_position.sql) · drop legacy [`018_drop_overall_position.sql`](../scripts/amiga/sql/018_drop_overall_position.sql) · Tier E overrides [`019_tournament_finish_override.sql`](../scripts/amiga/sql/019_tournament_finish_override.sql) ([`amiga-tournament-honours-rules.md`](amiga-tournament-honours-rules.md)) · standings scope unification [`020_unify_league_standings_scope.sql`](../scripts/amiga/sql/020_unify_league_standings_scope.sql) ([`amiga-standings-scope-policy.md`](amiga-standings-scope-policy.md)) · tournament medals v2 totals [`021_tournament_medals_totals.sql`](../scripts/amiga/sql/021_tournament_medals_totals.sql) · WC finish backfill [`021b_wc_finish_backfill.sql`](../scripts/amiga/sql/021b_wc_finish_backfill.sql) · drop `wc_medal` [`022_drop_wc_medal.sql`](../scripts/amiga/sql/022_drop_wc_medal.sql) ([`amiga-tournament-honours-rules.md`](amiga-tournament-honours-rules.md) v2, **complete** Jun 2026) · stage type unification [`023_unify_stage_types.sql`](../scripts/amiga/sql/023_unify_stage_types.sql) ([`amiga-tournament-structure-policy.md`](amiga-tournament-structure-policy.md) slice 1). Contract: [`amiga-player-universe-contract.md`](amiga-player-universe-contract.md) §5.2–§5.3. Website read path: [`includes/amiga_db.php`](../site/public_html/includes/amiga_db.php), tournament pages [`includes/amiga_tournament_lib.php`](../site/public_html/includes/amiga_tournament_lib.php), participation [`includes/amiga_player_tournament_lib.php`](../site/public_html/includes/amiga_player_tournament_lib.php).
...
- `tournament_stages` and `tournament_fixtures` are **ground** for both live and legacy tournament structure once materialized. Legacy Access imports leave them empty until `tournament-structure materialize` or a curated `StructureSpec` apply path runs.
