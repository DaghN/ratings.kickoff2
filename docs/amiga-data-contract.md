...
| `amiga_player_inverse_count_at_event` | Derived | Tournament finalize / `replay` — sparse changelog for four inverse counts (`mgs_culprits`, `bw_culprits`, `mgc_victims`, `bl_victims`); ghost events when credit transfers while hero absent; TT/mosaic/Victims LB authority — [`amiga-player-inverse-count-timeline-policy.md`](amiga-player-inverse-count-timeline-policy.md) | **Active** |
| `amiga_player_current` | Derived | Tournament finalize / `replay` — present projection (career + honours + geo/year scalars + rise dates + **`peak_elo_rank`** denorm + inverse counts synced from changelog); **no `wc_*`** (WC → slice tables) | **Active** |
