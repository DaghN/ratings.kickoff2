...
- **Public newcomer registration is deferred.** Internal ops use `python -m scripts.amiga players`:
  ...
  - `players create --name TEXT [--country TEXT] [--dry-run]` — insert one ground-truth player row (`display=1` by default). Refuses identity/exact collisions. Does **not** create `tournament_entrants`; register entrants separately via fixture tooling.
- Player creation for live events is separate from entrant registration (`fixtures replace-entrant` still refuses to create players).
